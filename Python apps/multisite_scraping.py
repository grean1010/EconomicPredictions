...
def scrape_bonds():
    ...
    dow = float(stocks[3].text)
    snp = float(stocks[0].text)
    nasdaq = float(stocks[6].text)
    ...
    eco_scrape_dict['dow'] = f'Current Dow Jones Index trading volume: {dow}'
...
def scrape_unemployment():
    ...
    unemployed = soup.find_all('div', class_ = "Z0LcW AZCkJd")
    unemployment = float(unemployed[0].text)
...
def scrape_housesales():
    ...
    house_price = float(price[0].text)
...
def scrape_earnings():
    ...
    # Store data in eco_scrape_dict{}
    eco_scrape_dict['earnings'] = f'Current earnings in the US is {z}'
